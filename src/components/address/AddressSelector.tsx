import { District, Province, Ward } from "@/models/address/address.model";
import { AddressLevel } from "@/utils/enum";
...
export default function AddressSelector(props: {
  ...
  const fetchAdminBoundaries = async () => {
    try {
      ...
      let adminBoundaries = await data.json();
    ...
    <div className="p-4 flex flex-col h-full">
      <div>
        ...
        <div>
          ...
          {selectedAddresses.length < 3 && (
            <div className="rounded-full border border-gray-500 overflow-hidden p-1 px-3 flex items-center">
              <input
                type="text"
                name=""
                id=""
              ...
              />
              <button className="w-10 aspect-square text-white bg-primary rounded-full">
        ...
        {selectedAddresses.length === 3 && (
          <div>
            ...
            <textarea
              ...
              className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-yellow-100 focus:border-primary"
            ></textarea>
          </div>
        )}
      </div>
