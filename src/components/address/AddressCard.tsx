  ...
  return (
    <div className="p-4 bg-white flex items-center justify-between">
      <div className="flex items-center space-x-5">
        <div
          className={`${
            infoAddress.defaultAddress ? "bg-red-100" : "bg-slate-100"
          } h-12 w-12 rounded-full flex justify-center items-center`}
        >
          {infoAddress.typeAddress === 1 ? (
            <FontAwesomeIcon
              className={`${
                infoAddress.defaultAddress ? "text-red-600" : ""
              } h-5 w-5`}
              icon={faHouse}
            />
          ) : (
        ...
        <div>
          ...
          <div className="text-[#757575]">
            {infoAddress.ward}, {infoAddress.district}, {infoAddress.province}
          </div>
        </div>
      </div>
      <div>
        <div className="text-end">
          <Button onClick={onOpen} color="primary" variant="text">
            Cập nhật
          </Button>
          {!infoAddress.defaultAddress && (
